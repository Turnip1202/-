...
async function getPageUrl(){
  num++;
  let httpUrl = 'https://www.aibooks.cc/page/'+num;
...
async function getBookInfo(href,num){
  ...
  // 拿到标签
  let tag = $('.book-left .bookinfo li:nth-child(7)').text();
  tag = tag.slice(3);
  // 拿到时间
  let pubtime = $('.book-left .bookinfo li:nth-child(6)').text();
  pubtime = pubtime.slice(4);
  // 拿到评分
  let score = $('.book-left .bookinfo li:nth-child(4)').text();
  score = score.slice(3);
  // 拿到出版社
  let publisher = $('.book-left .bookinfo li:nth-child(5)').text();
  publisher = publisher.slice(4);
  ...
  */
  ...
  let download = $('body > section > div.content-wrap > div > article > table > tbody > tr:nth-child(3) > td > a:nth-child(4)').attr('href');
